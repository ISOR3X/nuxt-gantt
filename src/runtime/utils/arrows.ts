import { TaskDependencyType } from "../types/gantt";
...
export function getAnchors(
  ...
  const fromLeftX = fromCol * cellSize.width;
  const fromRightX = (fromCol + fromWidth) * cellSize.width;
  const fromCenterY = fromRow * cellSize.height + cellSize.height / 2;

  const toLeftX = toCol * cellSize.width;
  const toRightX = (toCol + toWidth) * cellSize.width;
  const toCenterY = toRow * cellSize.height + cellSize.height / 2;
